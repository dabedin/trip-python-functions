...
az functionapp deployment source config-zip -g $resGroupName -n $funcAppName --src private\deploy.zip
...
az functionapp deployment source config-zip -g $resGroupName -n $funcAppName --slot $funcSlotName --src private\deploy.zip
